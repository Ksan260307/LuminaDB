...
### v1.35 — Japanese / English UI, defaulting to Japanese

A **JP / EN** switch in the sidebar header changes the interface language.
The choice is kept in `localStorage`, so the next visit opens in the same language.
Adds `test-suite-v64.js` (34 tests); the suite now totals 52,655.

**How it works**

- **Japanese is the source text.** The HTML contains Japanese directly; English is substituted
  from a dictionary ([`js/ui/i18n-en.js`](js/ui/i18n-en.js), 330 entries) only when English is selected.
  With JavaScript off, the page still reads.
- Dictionary keys **are the Japanese source strings**. Inventing separate key names would mean
  maintaining the HTML and the dictionary in parallel, and one of them would drift.
- Static text is picked up by walking the DOM. **An element whose children are only
  `<span>` / `<code>` / `<kbd>` is translated as a single unit (its innerHTML)**, so a sentence with
  markup in the middle never gets chopped into fragments.
- Text built in JavaScript goes through `i18nT('日本語')`. Arguments fill `{0}` placeholders —
  `i18nT('保存エラー: {0}', e.message)` — and **the translation may reorder them**
  (「{0} 表中 {1} 表を書き込み」 → "{1} of {0} tables written").
- Originals are stashed in a WeakMap before substitution, so switching back and forth is lossless.

**What is not translated**

| Subject | Reason |
| --- | --- |
| Table and column names, result cells, query history, the execution log, the ER diagram | That is the user's data, not interface text. Regions marked `data-i18n-skip` are never walked |
| SQL engine error messages | They are English already, and a database error is easier to search for when it does not change with a UI setting |
| Toasts already on screen | They are composed in the language current at the time; re-translating a past notification is meaningless |

**Traps hit along the way**

- Naming the translate function `t()` shadowed it wherever the codebase uses `t` for a table or a
  tab — a **silent `TypeError`**. `tr()` then collided with `<tr>` element variables, so it became `i18nT()`.
- An element that has been switched to English no longer looks Japanese, so **anything with a stored
  original must stay in scope regardless of its current language** — otherwise it can never go back.
- The dictionary is written with newlines for readability, but lookups use whitespace-collapsed keys.
  Without collapsed aliases, **only the multi-line SQL examples silently went untranslated**.

**New tests** (`test-suite-v64.js`) — Japanese is the default; the switch reaches `localStorage` and
`<html lang>`; **no Japanese remains in English mode** (checked mechanically across the main screen,
every modal, all five data-screen tabs, and the command reference); ja → en → ja restores the original;
five round trips leave it intact; `i18nT` argument substitution and fallback for unknown strings;
placeholder counts matching between source and translation; and table names, result cells and engine
errors staying untranslated.
