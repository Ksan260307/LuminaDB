    ...
    Object.assign(DatabaseEngine.prototype, {
      ...
      // 結合キー 1 個ぶんの「照合候補キー」。ハッシュ索引（Table.indices）が使う
      // 鍵と同じものでなければならない——違うと、索引を組み上がりのハッシュ表として
      // 借りている経路（下の keySpecs.length === 1 の分岐）が別の鍵で引くことになる。
      // 定義は 1 か所（Table.indexKeysOf）に置いてある
      _joinKeysOf(v) {
          return Table.indexKeysOf(v);
      },
      ...
      _optimizeSelect(parsed, isExplain, strMap) {
          ...
          if (whereStr) {
              ...
              if (col && !collated(col) && (!tblAlias || tblAlias === baseAlias || tblAlias === fromTable) && baseTbl.indices[col] && isLiteralVal) {
                  ...
                  // WHERE はここで捨ててよい。索引の鍵は型寄せのぶん広めに当たるが
                  // （'2' と ' 2 ' が同じバケットに居る）、この形の WHERE は
                  // 「列 = リテラル」そのものなので、実行側が拾った候補を
                  // Table.valueEq で直に検算する（式を回さないぶん点引きが速い）
                  residualWhere = null;
              }
                  ...
                  if (conj.length > 1) {
                      ...
                      if (cands.length > 0) {
                          ...
                          cands.sort((a, b) => {
                              ...
                              return baseTbl.indexCardinality(b.col) - baseTbl.indexCardinality(a.col);
                          });
                          isIndexScan = true;
          ...
          const exJoinFanout = (join, rightCol) => {
              ...
              if (rightCol && jt.indices[rightCol]) {
                  const b = jt.indexCardinality(rightCol);
          ...
          if (isExplain) {
              if (isIndexScan) {
                  ...
                  const buckets = idx ? this.tables[fromTable].indexCardinality(indexScanCol) : 0;
      ...
      _executeSelectPlan(plan, strMap) {
          ...
          if (isIndexScan) {
              ...
              // 索引の鍵は Table.indexKeysOf が張る同値類（比較側の __cpair と同じ寄せ方）。
              // 引く側も同じ関数で鍵を出せば、宣言型の名前を数え上げなくても
              // BOOLEAN 列への `ok = 1` も INT 列への `g = '2'` も素直に当たる。
              //
              // 鍵は「取り落とさない側」に倒してあるので余計な行も拾う（'2' と ' 2 ' は
              // 同じ 'n:2' を持つが __eq では等しくない）。拾った候補は必ず検算して、
              // 索引を使ったせいで答えが変わることがない、という性質を保つ:
              //   点引き   … この場で Table.valueEq（WHERE が `列 = リテラル` そのもの）
              //   IN / 連言 … residualWhere に残した元の WHERE で後段が検算する
              const probeVals = indexValList ? indexValList.map(toVal) : [toVal(indexValStr)];
              const keys = [];
              for (const v of probeVals) {
                  const ks = Table.indexKeysOf(v);
                  if (ks) for (const k of ks) keys.push(k);
              }
              // 点引き（`col = <リテラル>`）は WHERE を残していないので、ここで検算する。
              // IN (...) と連言の経路は residualWhere に元の式が残っているので後段に任せる
              const verifyVal = indexValList ? undefined : probeVals[0];
              const doVerify = !indexValList;
              const seenRow = keys.length > 1 ? new Set() : null;
              for (const key of keys) {
                  const matchedIndices = baseTbl.indices[indexScanCol].get(key);
                  if (!matchedIndices) continue;
                  for(let i of matchedIndices) {
                      if (seenRow) { if (seenRow.has(i)) continue; seenRow.add(i); }
                      if (doVerify && !Table.valueEq(baseTbl.getValue(indexScanCol, i), verifyVal)) continue;
          ...
          joinPlans.forEach(join => {
              ...
              let newPtrs = [];
              // 単一の等価 ON（join.isHashJoin）も複合キー（join.hashKeys）も同じ machinery で
              // 回す。以前は単一等価だけ別実装で、右表の値を**生値のまま** Map の鍵にして
              // 検算もしていなかったので、`a.k = b.k` が入れ子ループなら結合するのに
              // ハッシュだと結合しない、という食い違いがあった（INT の 2 と TEXT の '2'）
              const keySpecs = join.hashKeys
                  || (join.isHashJoin ? [{ leftAlias: join.leftAliasMatch, leftCol: join.leftColMatch, rightCol: join.rightColMatch }] : null);
              if (keySpecs) {
                  // ハッシュは「候補を絞る網」でしかない。__eq の型寄せをキー側で
                  // 完全に再現するのは難しいので、_joinKeysOf は寄せの候補を多めに
                  // 出しておき（取り落としが無い側に倒す）、拾った候補は必ず検算する。
                  //
                  // 鍵が 1 列なら組み合わせ鍵は候補キーそのもの。こうしておくと
                  // 右表の索引（同じ鍵で張ってある）を組み上がりのハッシュ表として borrow できる
                  const oneKey = keySpecs.length === 1;
                  // ON が「ちょうど 1 個の等価」だけのとき（join.hashKeys が無い経路）は、
                  // 検算も「両辺の値が __eq で等しいか」で足りる。式を組み立てると
                  // 別名検査・照合順序・日付列の書き換えという文字列処理が結合 1 回ごとに走り、
                  // 小さな結合を何千回も回すとそれが支配的になる。
                  // hashKeys 側は残余条件（`... AND s.b = 0`）を持ち得るので必ず ON 式で検算する
                  const fastEq = !join.hashKeys;
                  const onFunc = fastEq ? null
                      : this.compileCondition(this._applyDateColumns(this._applyColumnCollations(this._assertNoAmbiguousColumns(join.onCond, aliases), aliases), aliases), strMap);
                  const combosOf = (perCol) => oneKey ? perCol[0] : this._joinKeyCombos(perCol);
                  let rightMap;
                  if (oneKey && jTbl.indices[keySpecs[0].rightCol]) {
                      rightMap = jTbl.indices[keySpecs[0].rightCol];
                  } else {
                      rightMap = new Map();
                      for (let j = 0; j < jTbl.rowCount; j++) {
                          ...
                          for (const combo of combosOf(perCol)) {
                  ...
                  const rightCol1 = fastEq ? keySpecs[0].rightCol : null;
                  rowPtrs.forEach(ptr => {
                      const perCol = [];
                      let hasNull = false;
                      let leftVal1 = null;
                      for (const k of keySpecs) {
                          ...
                          const lv = lt.getValue(k.leftCol, ptr[k.leftAlias]);
                          const ks = this._joinKeysOf(lv);
                          if (!ks) { hasNull = true; break; }
                          leftVal1 = lv;
                          perCol.push(ks);
                      }
                      ...
                      if (!hasNull) {
                          ...
                          if (combos.length === 1) {
                              ...
                              // 実際に当たるバケットが 1 本だけのことが多いので、
                              // その場合は組み直さずそのまま使う（バケットは行番号の昇順）
                              let only = null, many = false;
                              for (const combo of combos) {
                                  const c = rightMap.get(combo);
                                  if (!c || c === only) continue;
                                  if (only === null) only = c; else { many = true; break; }
                              }
                              if (!many) {
                                  cand = only || [];
                              } else {
                          ...
                          for (const j of cand) {
                              tickH();
                              if (fastEq) {
                                  if (!Table.valueEq(leftVal1, jTbl.getValue(rightCol1, j))) continue;
                                  newPtrs.push({ ...ptr, [join.table]: j, [join.alias]: j });
                              } else {
                                  const combPtr = { ...ptr, [join.table]: j, [join.alias]: j };
                                  if (!onFunc(combPtr, this.tables, aliases)) continue;
