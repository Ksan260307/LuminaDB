...
### v1.42 — 用語をひとつに決め、スキーマ編集の穴を塞いだ（8 項目）

用語・フォームとしての完成度・グリッドの耐久性を見た回です。
**v1.39 で入れた安全装置が、いちばん危ない操作でだけ誤作動していた**のが最大の収穫でした。

**スキーマ編集で、列名を空にすると列が黙って消えていた**

列名の入力欄を空にして保存すると、その列が保存対象から外れて消えます。実測では

```
操作前 : CREATE TABLE users (id INTEGER PRIMARY KEY NOT NULL, name TEXT, age INTEGER)
操作   : 先頭列（id）の名前欄を空にして「スキーマを保存」
操作後 : CREATE TABLE users (name TEXT, age INTEGER)
```

主キーが 10 行分の値ごと消え、モーダルは成功したかのように閉じました。主キーが
無くなるので、セル編集・`＋ 行`・`－ 行`・ER 図の線も一緒に死にます。

さらに悪いことに、v1.39 で入れた「保存すると元に戻せません」の予告が、この経路では

```
出ていた文: 「列名を変えます: id → 。この名前を使っているビュー・トリガーは追随しません。」
```

と、**削除ではなく改名として説明していました**（分類が `oldName !== newName` を先に見るため）。
保護のつもりで入れた仕組みが、最も危険な操作で誤った説明を出していたことになります。

- 空の名前は入力ミスとして保存を止める（消したいなら削除の操作がある）
- 予告の分類を直し、空欄も「列 「id」 を削除します（10 行分の値が失われます）」と出す
- 列名が識別子として妥当かを検証する。`1abc スペース` のような名前が通ってしまい、
  あとから `PRIMARY KEY constraint failed` という無関係なエラーになっていた。
  **同じアプリの CSV 取り込みは同じ判定で正しく弾いていた**ので、そちらに揃えた
- 入力欄に `required` / `pattern` / `maxlength` / `aria-label` を付けた

**長い値ひとつで結果グリッドが崩れていた**

列幅に上限が無く、`white-space: nowrap` / `text-overflow: clip` / `max-width: none` の
ままだったので、400 文字のセルが 1 つあるだけで表全体が横に伸びていました。

| 400 文字のセルを含む 3 列の表 | 直す前 | 直した後 |
|------------------------------|--------|----------|
| そのセルの幅 | 4,833px | 448px |
| 表全体の幅 | 4,976px | 624px |
| 3 列目 | 画面外 | 見える |

400 文字は説明欄・JSON・ログ行なら普通の長さです。全文はセルの詳細
（クリック / Enter）で読めるので、グリッド側は省略表示にして一覧性を優先します。
省略された値は `title` でも読めます。

**用語がひとつに決まっていなかった**

画面に出る日本語を数えたところ、同義語が両方使われていました。

| 正規にした語 | 混在していた語 | | 正規にした語 | 混在していた語 |
|---|---|---|---|---|
| 表 88 | テーブル 11 | | 初期化 6 | リセット 3 |
| 列 49 | カラム 6 | | 削除 16 | 消去 2 |
| 取り込み 11 | インポート 4 | | 書き出し 29 | エクスポート 1 |

サイドバーの見出しは「テーブル」、空のときは「表がまだありません。」、
不在メッセージは「テーブル 'x' がありません。」と、同じ画面で 3 通りでした。

さらに、**同じ事象の文言が隣り合う行で割れていました**。

```js
showImportLog([i18nT('SQL インポート失敗: {0}', ...)], true);   // 空白あり
showToast(   i18nT('SQLインポート失敗: {0}', ...), true);       // 空白なし
```

辞書にも空白違いの重複キーが 2 行並んでおり、1 回の失敗でログとトーストに
別々の日本語が同時に出ていました。正規の語を **表 / 列 / 取り込み / 書き出し /
初期化 / 削除** に決め、22 箇所を揃えて辞書のキーも合わせ、重複を 1 つに畳みました。

（「中止」＝実行を止める と「キャンセル」＝ダイアログを閉じる、
「行数」＝追加する行の数 と「件数」＝結果の件数 は意味が違うので残しています）

**キーボードでエディタに着くまで 24 回かかっていた**

主作業面であるエディタは Tab 順で 25 番目でした。サイドバーは表 1 つにつき
3 つの操作を持つので、表 20 個なら約 66 回になります。フォーカスしたときだけ
見える「クエリ入力欄へ移動」を先頭に置きました（Tab 1 回でエディタへ）。

**青が 3 つの意味を持っていた**

主要操作の地色・数値セルの文字色・編集可バッジがすべて青系で、グリッドを
眺めたときに型の表現と状態の表現が混ざっていました。バッジを緑系へ分けています。

### v1.41 — 画面の使い方そのものの見直し（11 項目）

不具合ではなく**設計**を見た回です。「画面の面積をどう配るか」「最初の 1 分で
何を見せるか」「危ない操作をどう伝えるか」の 3 つが手つかずでした。
このうち 2 件は v1.39〜v1.40 の修正が生んだ副作用です。

**結果に画面の 35% しか割いていなかった**

1280×720 の実測で、データを見る前に 403px（画面の 56%）が枠に消えていました。
原因は結果ツールバーで、**1280px 幅では 2 段に折り返して 131px** を占めます
（1440px 以上なら 57px）。v1.39 で「1024px で表示件数・履歴・? が画面外へ出る」を
折り返しで直したのですが、画面外に出る問題を太いツールバーに置き換えただけでした。

根っこは数です。11 個のうち **5 個が「結果を外へ出す」系**（CSV / JSON / Markdown /
TSV / INSERT）。v1.24 でサイドバーの 4 ボタンを「データ」1 つへ集約したのと
同じ整理を、結果ツールバーだけが受けていませんでした。「書き出し ▾」1 つへ畳みます。

| 1280×720 | 直す前 | 直した後 |
|----------|--------|----------|
| 結果ツールバー | 131px（2 段） | 53px（1 段） |
| 結果領域 | 317px（画面の 35%） | 395px（画面の 55%） |

サイドバーも、表 1 つにつき 4 個の操作（展開・選択・列プロファイル・スキーマ編集）が
並んでいて、20 表なら 80 個を超えていました。常時出していた icon 2 個を「⋯」1 つへ
まとめます。**操作は 1 つ減り、届く機能は 2 → 8 に増え**、それまで見つけようの
無かった右クリックメニューが表から見えるようになります。

**最初の 1 分に何も無かった**

初回訪問を再現すると、エディタは空、41 個のボタンだけが出て、画面でいちばん
大きい「実行」を押しても（空文なので）**無反応**でした。500 以上の構文・ER 図・
列プロファイル・ワーカー実行があるのに、入口がどこにも示されません。

- 初回だけ、サンプル 3 表を結合する 1 文をエディタへ置く。押すだけで結果グリッド・
  編集可バッジ・件数・実行時間の意味が同時に伝わります（2 回目以降は出ません）。
- 空のまま実行したら一言返す。

サイドバーの表をクリックしたときの挙動も直しました。**左クリック（見つけやすい）は
SQL を入れるだけで実行せず、右クリック（見つけにくい）だけが実行まで済ませる**、
という逆転になっていました。生成する SQL も `LIMIT` の有無が食い違っていました。

**全行を書き換えても、成功にしか見えなかった**

`UPDATE users SET age = 0`（WHERE なし）は `NO_WHERE` 警告を出しており、コンソールへも
届いていました。ところが受け皿のコンソールは既定で閉じており、ランチャーは
**通常ログ 5 件でもエラーでも全行削除でも同じ濃いグレー**（変わるのは数字だけ）。
つまり誰も開いていない場所にだけ出ていたことになります。

- ランチャーとバッジを深刻度で塗る（警告=琥珀・エラー=赤、未読の件数）。開けば戻る。
- WHERE なしの UPDATE / DELETE / TRUNCATE は、**実行の直前に内部スナップショットを取り**、
  終わったら「元に戻す」付きのトーストを出す。取り消し手段が実質無く（トランザクションは
  手で開いた人だけ、スナップショットは SQL からしか作れない）、自動保存が 1 秒後に
  確定させる、という状態だったため。退避は `__` 始まりの名前なので `SHOW SNAPSHOTS`
  には出ません。DB が大きすぎて退避を取れないときは、取らなかったことを伝えます。

**Help が探せなかった**

60 カテゴリ・590 項目・縦 41,730px（表示領域の 105 画面分）を、目次なしで
スクロールする作りでした。さらに、

- カテゴリ名 60 個が**すべて英語**（日本語表示でも `Basic DQL` のまま）
- 検索が英語前提で、`結合` 0 件・`並べ替え` 0 件。`日付` が 6 件当たるのは、
  たまたま説明コメントにその語が入っていたからで、設計ではありませんでした

カテゴリへ飛べる目次を付け、60 個の見出しを訳し、日本語で探せるよう言い換え表
（58 語）を通しました。`結合` → 73 件、`並べ替え` → 63 件、`主キー` → 9 件。

**10px の日本語**

画面に文字サイズ×太さが 12 種類あり、最小は 10px。`Time` / `Rows` / `Limit` は
10px のラテン小見出しとして成立していましたが、**v1.40 で日本語に置き換えたことで
読みづらさが生まれました**（CJK は同じ級数でも字面が複雑で、10px は実用下限を割る）。
日本語を載せるラベルは 12px へ上げ、日本語に効かない `uppercase` を外し、
最後まで英語で残っていた区分見出し `Tables` を「テーブル」にしました。

### v1.40 — 使用体験の再点検（10 項目）と、その過程で見つけた 2 つの地雷

v1.39 の続きで、まだ触っていなかった経路（一括取り込み・広い結果・タブ・
右クリックメニュー）を操作して見つけた 10 件を直した回です。途中で、
**表に出ていなかった重い不具合を 2 つ**掘り当てています。

**多行 INSERT が二乗だった（最大 438 倍の改善）**

`_parseValueToken()` は値トークン 1 個ごとに strMap を全件走査し、要素ごとに
`new RegExp` を作って `replace` を掛けていました。値の個数 × 退避した文字列の
個数で効くので、多行 INSERT は完全に二乗です。10 列 400 行（リテラル 4,000 個）で
3.2 秒、800 行で 13.6 秒。同じ形でも**値が数値なら 5ms** だったので、
遅さはすべてここに由来していました。

トークンは `__STR_12__` の形と決まっているので、出てきた番号で引けば一巡で済みます
（`_restoreStrings` が元からその実装）。

| 10 列 | 直す前 | 直した後 |
|-------|--------|----------|
| 400 行 | 3,213 ms | 25 ms |
| 800 行 | 13,592 ms | 31 ms |
| 2,000 行 | 30 秒以上 | 95 ms |

**列見出しの固定表示が一度も効いていなかった**

結果表を包む `<div>` に `overflow-hidden` が付いていました。overflow が visible
以外の要素は「スクロールの入れ物」になるので、中の `position: sticky` は
**スクロールしないその div** に貼り付き、実際にスクロールしている
`#resultsContainer` に対しては効きません。`sticky top-0` は当たっているのに
機能しておらず、300 行の結果を 400px 下げるだけで列名が画面外へ消えていました
（角を丸めるために付いていたクラスが、そのまま固定表示を殺していた）。

`#resultsContainer` は `p-4` なので、`top-0` のままだと余白の隙間から行が覗きます。
余白のぶん手前へ寄せて（`-top-4` / `-left-4`）、枠に密着させています。

---

**CSV 取り込みの上限（1 件目）**

`_writeRows()` が全行を 1 本の INSERT に畳んでいたため、プレースホルダの並びが
100 万文字の上限に達し、3 列で約 90,900 行 / 10 列で約 31,200 行 / 40 列で
約 8,200 行を超えると丸ごと失敗していました。50,000 行・1.1MB の CSV が
`Query too long (max 1,000,000 characters).` という、取り込みとは関係の無い
文言で拒否されます。

行を複数の文へ分けます。長さは **バインド後** で見ること（`query()` は `?` を
実際の値のリテラルへ置き換えてから組み立てるので、プレースホルダの数で区切ると
値の長い表で結局あふれる）。複数文になるときは 1 つのトランザクションで包み、
「全部入るか、何も入らないか」を保ちます。

いま通るようになったもの: 60,000 行 × 10 列（4.3 秒）/ 20,000 行 × 40 列（5.2 秒）/
5,000 行 × 10 列で値 200 字（0.7 秒）。実際の取り込み画面から 50,000 行の CSV も通ります。

**時間のかかる処理に進行中の合図（2 件目）**

`data-io.js` には `disabled = true` も `aria-busy` も 1 つもありませんでした。
ダミー行 10 万件の生成は 4 秒強かかるのに、ボタンは押せたまま・表示も変わらないので、
反応が無いと思ってもう一度押せてしまいます（二重に生成される）。
クエリ実行と同じ扱いへ揃え、生成・CSV 取り込み・SQL/JSON 書き出し・保存・読み込みに
「処理中」の表示とボタンの無効化を入れました。

**書きかけの SQL を潰さない（4 件目）**

Help の Use、サイドバーの表、ER 図、列プロファイルは、どれも書きかけを黙って
上書きしていました。Ctrl+Z で戻せはしますが、そう案内する場所がありません。
人が打った内容が残っているときは潰さずに新しいタブで開きます。
表を続けて選ぶ操作でタブが増え続けないよう、「この仕組みが入れた内容のまま
触られていない」場合は同じタブを使い回します。

**そのほか**

- 書き出しの対象が画面と一致しない（表示件数 100 でも 5,000 行出る）のに、
  どこにも書いていなかった。絞り込みには従う、という規則を注記で明示した。
- 「件数」が絞り込みに追随せず、脚注（15 / 15 件を表示）と画面の中で食い違っていた。
- ワーカー実行に `aria-busy` が無く、Cancel は出るのに支援技術には進行が伝わらなかった。
- `1 rows updated.` — DML のたびに出る、いちばん目にする文字列の単複が崩れていた。
  件数付きの状態メッセージを 1 か所（`_rowsMsg`）へ寄せた。
- セル詳細（Type: / Length: / Copy / Close）、列プロファイルの見出し行、
  ER 図の件数表示が英語のままだった。ツールバーの動詞（Format / Run / Export CSV …）も
  原文を日本語へ揃え、英語は辞書側で与えるようにした。
- 結果セルの詳細・編集がマウス専用だった。矢印で移動・Enter で詳細・F2 で編集。
- エディタタブが div のままで、閉じる × だけが focusable という非対称だった。
  tablist / tab として宣言し、← → で移動・Enter で選択・F2 で改名・Delete で閉じる。
- 表の右クリックメニューに role が無く、開いてもフォーカスが移らなかった。
  menu / menuitem を宣言し、↑↓ Home End で辿れて、閉じたら開いた場所へ戻す。

**`tools/build-tailwind.mjs`**

負の値のユーティリティ（`-top-4` など）は先頭が `-` なので、接頭辞の照合から
漏れて候補に入りませんでした（UTIL の正規表現は元から `-` 始まりを許しているのに、
ここだけ弾いていた）。接頭辞を見る前に `-` を外すようにしています。

### v1.39 — 画面の使用体験の総点検（18 項目）

実際にアプリを操作して見つけた 18 件を、まとめて直した回です。エンジンの完成度に対して
「初回の見え方」と「画面幅への耐性」が追いついていない、というのが全体の所見でした。
テストは既存スイートへ足して 53,276 件のまま全件通ります。

**サンプルデータに型・主キー・外部キーを宣言した**

起動時の 3 表は列を足すだけで作っており、型も鍵もありませんでした。その結果、初回に
触れる範囲で次の 4 つが揃って「壊れている」ように見えていました。

| 機能 | 直す前に見えていたもの |
|------|------------------------|
| セルのダブルクリック編集 | 全表で無効（行を特定する鍵が無い） |
| `+ Row` / `− Row` | 常に無効 |
| ER 図 | 「3 表 / 外部キー 0」。線が 1 本も出ず、全列 `ANY` |
| `SHOW CREATE TABLE` | `CREATE TABLE users (id, name, age)` |

画面下のヒントは「セルをダブルクリックで編集」と案内しているのに、案内された操作が
どの表でも効かない状態でした。`INTEGER` / `TEXT` と `PRIMARY KEY`、`orders` から
`users` / `products` への外部キー（`ON DELETE CASCADE`）を宣言し、4 つとも動くようにしています。

**補完ポップアップが Ctrl+Enter を奪っていた**

語の途中で `Ctrl+Enter` を押すと、実行されずに候補が挿入されて SQL が黙って書き換わって
いました（修飾キーを見ずに `Enter` を捕まえていたため）。最頻出のショートカットが
最頻出の状況で効かない状態です。修飾キー付きは補完へ渡さないようにし、あわせて
`Enter` での確定は **↑↓ で明示的に選んだときだけ**にしました（素の改行が打てなかった）。
`FROM` / `JOIN` の直後は表名だけを出すようにもしています（`FROM us` に `USING` や
`USER_VERSION` が並んで本命が埋もれていました）。

**トーストが消えた後も透明な当たり判定を残していた**

`showToast()` が `className` を丸ごと差し替える際に、HTML 側に付けてあった
`pointer-events-none` を落としていました。一度でもトーストが出ると、以降セッション中ずっと
結果エリアの右下に見えないクリック吸収帯が残ります。原因が見えないので「たまに効かない」と
しか認識できない不具合でした。

**画面幅への耐性**

結果ツールバーが折り返しも横スクロールも持たず、`1024px` の時点で **表示件数の選択・履歴・`?`**
が画面外へ出ていました。`body` が `overflow-hidden` なのでスクロールでも拾えず、とくに
表示件数が消えると「結果は 100 件のまま、変える手段が無い」状態になります。折り返しを入れ、
`768px` 未満ではサイドバーを引き出しに変えました（それまでは `375px` 幅で本体に 119px しか
残らず、Run ボタンごと画面外でした）。

**結果とエラーの見せ方**

- 複数文でエラーが 1 件でもあると、成功した文の結果セットを**全部捨てて**要約だけ出していた。
  失敗した文もタブに混ぜ、成功分は隣のタブに残るようにした。
- 5 万行の結果でも最初は 50 行しか描かないのに、**続きがあることを示す表示が無かった**。
  「50 / 10,000 件を表示 — 下へスクロールすると続きを読み込みます」を出す。
- 「読み取り専用」の理由がツールチップの中だけだった。バッジ本体に出し、JOIN のときに
  「run a SELECT first」という誤った理由を返していたのも直した。
- メインスレッド実行に進行中の表示が無く、重いクエリでは既定 30 秒の上限まで無反応だった。
  実行中を出し、1.5 秒を超えた参照系ではワーカー実行を案内する。
- 表が 1 つも無いとき、サイドバーが無言の空欄だった（案内は検索中にしか出していなかった）。

**日本語表示なのに見出しと主要ボタンだけ英語だった**

原文＝日本語・英語は訳、という方針に対して、HTML に英語を直書きした箇所は
**日本語表示のときだけ訳されずに残ります**。モーダル見出し全部（`Table Editor:` /
`Command Reference` / `Query History` ほか）、主要ボタン（`Save Schema` / `+ Add Column` /
`Clear Data`）、`Time` / `Rows` / `Limit`、`Showing top ... rows out of ...` などが該当しました。
原文を日本語へ寄せ、英語は辞書へ入れています。`(1 errors)` の単複の崩れも
`i18nPlural()` で一箇所に寄せました。

このとき、翻訳単位が実行時に中身の変わる `span`（件数・表名）を巻き込むと見出しが
永久に訳せなくなることが判ったので、`data-i18n-skip` を単位の壁として扱うようにしています。

**アクセシビリティ**

- 9 つのモーダルに `role="dialog"` / `aria-modal` / `aria-labelledby` が無く、フォーカスも
  動かしていなかった。開いたら中へ（検索欄があればそこへ）、閉じたら呼び出し元へ戻し、
  `Tab` で背後へ出られないようにした。`Esc` は 1 回で 1 枚だけ閉じる（以前は全部閉じていた）。
- 表の行の「列プロファイル」「スキーマを編集」が `opacity-0` + `group-hover` でしか
  現れず、**タッチ環境とキーボードから到達できなかった**。既定を薄い表示にし、
  hover とフォーカスで濃くする。`aria-label` も付けた。
- 列見出しの並べ替えがマウス専用だった。`tabindex` / `role="button"` / `aria-sort` を付け、
  `Enter` / `Space` で並べ替えられるようにした。
- トーストに `aria-live` が無く、支援技術に一切届いていなかった。使い回しの 1 要素なので
  消灯タイマーも持ち回るようにした（連続して出すと前のメッセージが早く消えていた）。
- 補完候補に `listbox` / `option` の役割が無かった。
- 白地に対して `gray-400`（約 2.6:1）・`gray-300`（約 1.5:1）を本文に使っており、
  AA の 4.5:1 に届いていなかった。`gray-500`（4.8:1）へ寄せた。とくに `[NULL]` と
  `[empty]` は、両者を区別するための印そのものが読めない状態だった。

**`tools/build-tailwind.mjs` が空回りしていた**

生成済み CSS を作るこのツールは、JIT（Tailwind の CDN）を動かすために
`LuminaDB.html` を開いていました。ところがツール自身の成果物が
「CDN の `<script>` を外して生成済み CSS を読む」ページなので、**2 回目以降は
JIT がどこにも居ません**。それでも「いちばん規則の多い stylesheet」を拾う実装
だったため、拾っていたのは生成済みの `css/tailwind.css` 自身でした。
既存の CSS をそのまま書き戻すだけで、新しいユーティリティは黙って欠落します
（その要素だけ素のまま表示される）。今回 `gap-y-2` と `border-amber-300` が
出てこないことで気づきました。

- JIT は Node 側で取得し（版を `3.4.17` に固定）、他に stylesheet を持たない
  **検査専用ページ**で動かす。リポジトリの外の一時ディレクトリに作って配る。
- 空回りを二度と黙って通さないため、`flex` / `hidden` / `bg-blue-600` /
  `hover:bg-gray-100` を目印として検査し、欠けたら失敗させる。
  規則にならなかった候補は一覧で出す。
- ポートが塞がっていると、前回の回し残りのサーバー（別のディレクトリを配っている）が
  応答し続けていた。配っているのが自分のページかどうかを中身で確かめ、駄目なら止まる。
- クラス名の収集が、`focus-within:` や `group-hover:` のように**ハイフンを含む修飾子**を
  弾いていた。元の版は実ページの DOM を JIT に見せていたので、落としても結果的に
  生成されていて表に出ませんでした。
- `bun tools/build-tailwind.mjs --check` を追加。ブラウザも網も使わず、
  いまの `css/tailwind.css` がいまのソースを賄えているかだけを見ます
  （`npm run check:css`）。回し忘れの発見用。

**その他**

- スキーマ編集の保存が、列の削除と型変換を無警告で実行していた。データ画面は破壊的操作を
  1 件ずつ説明しているのに、ここだけ予告が無かった。何が失われるかを保存前に出す。
- ショートカット一覧に `Ctrl+Alt+Enter`（ワーカー実行）が載っていなかった。
- 旧名の IndexedDB（`JSLiteDB`）が、移行するものが無い環境にも**作られて残っていた**
  （`indexedDB.open()` は無ければ作る）。存在確認してから開き、移行の成否によらず消す。
