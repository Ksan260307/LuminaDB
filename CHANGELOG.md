...
v1.37 の末尾に「まだ手を付けていないもの」として並べた 7 件を、すべて実装した回です。
テストは `test-suite-v71`（55 件）を追加し、既存スイートへも 30 件ほど足して
全体で 53,276 件になりました。

**全文検索に転置索引を入れた**

`MATCH ... AGAINST` は行ごとに本文を語へ切って走査する関数で、索引が無く表の
大きさに正比例していました。`CREATE FULLTEXT INDEX [IF NOT EXISTS] 名 ON 表 (列, ...)`
を受け付け、語 → 行の対応から候補を絞ります（4 万行）:
...
索引は候補を絞る網としてだけ使い、一致の判定は `MATCH` 式そのものが行います。
だから索引側は「取りこぼさない」ことだけ満たせばよく、句は語の積・前方一致は
接頭辞を持つ全語の和・`-語` は候補を狭めない、で足ります。正の語が 1 つも無い
問い合わせは候補を作れないので全表走査に落とします。
語の切り方は照合側と同じ関数（`LUMINA_FT_TOKENS`）を共有します。ここが食い違うと
「索引経路だけ行が消える」ので、1 箇所に置いて両方から使っています。
索引の作り直しは表の変更世代に連動する遅延方式なので、**書き込みのたびに作り直します**
（書いた直後の検索は 75ms）。増分保守は文単位ロールバックとの兼ね合いがあるので今後の課題です。

- **潜在していた欠陥を 1 件修正。** `_removeRows` は行を詰めて `rowCount` を更新するのに
  **変更世代を進めていませんでした**。世代で無効化する派生構造が古いままになり、
  消したはずの行が全文検索に出ていました。`TRUNCATE`・トランザクションの巻き戻し・
  文単位ロールバック・スナップショット復元にも同じ抜けがあり、すべて塞いでいます。

**SQL ダンプを 1 本の文字列にしない**

全文を配列へ積んで最後に `join` していたので、20 万行で 4.6MB の文字列が
配列とは別にもう 1 つできていました。本体を「1 文ずつ渡す」形に変え、
`exportSQL()`（従来どおり文字列）と `exportSQLParts()`（断片の配列）を用意しました。
`Blob` は断片の配列をそのまま取れるので、画面からの書き出しは連結を作りません。

**自動保存を列単位にした**

表 1 つを 1 レコードに書いていたので、1 セル直しただけでその表の全列を
直列化・base64 化・暗号化していました。列ごとにレコードを分け、指紋の変わった列だけ書きます:
...
文字列プールも列のレコードへ移しました（表側に残すとテキスト列のある表で毎回
プール全体を書き直すため）。形式は `chunked-v2`。`chunked-v1` も読めるので、
既に保存してある DB はそのまま開けて次の保存で移行します。

- **潜在していた欠陥をもう 1 件修正。** 変更世代を保存していなかったため、
  取り込みで採番が 0 からやり直しになり、**保存済みの指紋と偶然一致**して
  「変わっていない」と判定されることがありました（実際に 1 列が保存を飛ばされました）。
  表単位の指紋にも同じ穴がありました。世代をダンプへ含めて復元します。

**索引の無い列の相関 `EXISTS` を非相関化した**

内側が「1 表の等価 1 本」なら、答えは「その値がその列に現れるか」だけです。
文の中で 1 度だけ値の集合を作って所属を見ます:

| 条件 | 変化 |
|---|---|
| 相異なり 400 通り・索引なし | 4,830ms → **4.4ms** |
| 相異なり 50 通り・索引なし | 663ms → **8.1ms** |
| 相異なり 400 通り・索引あり | 21ms → **1.0ms** |

`__eq` の型寄せに合わせて候補値も引くので `7` が `'7'` を見つけます。
`NULL` は一致せず、照合順序付きの列は生値の集合では判定できないので従来経路のままです。

**式の中の加減乗も十進で合わせた**

`SUM` は v1.36 で桁ずらし整数にしましたが、式の中は二進小数のままでした。
十進で s1 桁と s2 桁の値なら、真の答えの桁数は加減で `max(s1,s2)`、乗で `s1+s2` に
必ず収まるので、そこへ丸めれば真の十進値へ戻ります。

| 式 | 変化 |
|---|---|
| `0.1 + 0.2` | 0.30000000000000004 → **0.3** |
| `price * 3 = 0.30` | false → **true** |
| `1.005 * 2` | **2.01** |

除算・12 桁を超える値・桁をずらすと整数の範囲を出る値は触りません。
最後の判定は最初 `1e15` で切っていて、`999999999999999.5` を **1 桁へ丸めると
逆に壊す**（10 倍した値が表せない）ことをテストが検出したので、桁をずらした後の
値そのもので判定するよう直しています。

**実行中のクエリを止められるようにした**

JavaScript は単一スレッドなので、メインスレッドで走っているクエリを Cancel ボタンで
止めることはできません（クリックが処理されない）。**Run ⧉**（Ctrl+Alt+Enter）で
ワーカーへ送ると、画面は固まらず、Cancel でワーカーごと終了できます。
書き込みは「どちらが本物か」が割れるのでメインスレッドの担当のままです。

**Tailwind を同梱し、バンドラから `import` できるようにした**

- CDN（`https://cdn.tailwindcss.com`）は静的な CSS ではなく実行時にクラス名を見て
  CSS を作る JIT なので、単に落としてくることができませんでした。
  `tools/build-tailwind.mjs` がヘッドレスブラウザで 1 度だけ動かし、ソースから集めた
  409 個のクラスをぶつけて生成させ、`css/tailwind.css`（418 規則・31KB）として書き出します。
  **入れ替えの検証**: 2,376 個の要素すべてについて 24 の描画プロパティの計算値を
  CDN 版と突き合わせ、差が出たのは 14 要素・すべて内容や状態の違い（トーストの中身、
  計測値の文字幅、無効状態のボタン）で、**描画プロパティの差はゼロ**でした。
  `LuminaDB.html` から外部ホストへの参照は無くなりました。
- `dist/luminadb.mjs` を追加（`tools/build-esm.mjs` が生成）。エンジンを 1 つの
  関数スコープで包んで `export` するので、Node / Bun / バンドラ /
  `<script type="module">` のどこからでも `import` できます。
  `window` などはスコープ内で束ねるだけなので、読み込んでも実行環境を汚しません。

---

### v1.37 — 行数に対して二乗・線形だった処理の作り直し

「答えは合っているが桁違いに遅い」箇所を、実測で裏を取って直した回です。
テストは `test-suite-v69`〜`v70`（134 件）を追加し、全体で 53,198 件になりました。

**ウィンドウ関数のフレーム集計が二乗だった（最大の修正）**

行ごとにフレームを配列として作り直し、そのたびに引数式をフレーム内の全行へ
評価していました。既定フレーム（`UNBOUNDED PRECEDING → CURRENT ROW`）だと
n 行目で n 行を見るので合計 n²/2 回。行数を倍にすると時間が 4 倍になり、
**累計や移動平均が実用になりませんでした**。

| 行数 | `SUM(x) OVER (ORDER BY id)` |
|------|------|
| 2,000 | 101ms |
| 8,000 | 1,690ms |
| 32,000 | **27,422ms → 26ms** |

引数値を 1 行 1 回だけ配列へ評価し、累積和・累積件数を持つことで、
フレームがどれだけ広くても 1 行あたり O(1) になりました。`MIN`/`MAX` は
下端が `UNBOUNDED PRECEDING` に貼り付く形を持ち回りで解きます。累積和は
桁ずらし整数で積むので、`DECIMAL` の累計も v1.36 の `SUM` と同じく十進で合います。
`EXCLUDE` 指定があるときは従来経路のままです。
**12 関数 × 19 フレーム指定の 228 通りを旧実装と突き合わせて完全一致を確認**しています。

**`LIMIT` が全行を組み立ててから捨てていた**

| 行数 | `SELECT * FROM t LIMIT 10` |
|------|------|
| 50,000 | 27ms → **1.9ms** |
| 400,000 | 164ms → **8.8ms** |

`COUNT(*)` が 4ms のままだったので走査は費用ではなく、40 万個の出力オブジェクトを
作って 39 万 9,990 個捨てていたのが費用でした。表の先頭を覗くのは画面でいちばん
多い操作です。並べ替え・グループ化・`DISTINCT`・ウィンドウ・集計・`WITH TIES` が
絡む場合は、後段で行が減る / 順番が変わるので従来どおり全行を見ます。

**範囲検索を索引で引けるようになった**

索引はハッシュ（Map）だけで等価一致しか引けず、`col > 100` や `BETWEEN` は
索引があっても全表走査でした。キーを並べた配列を遅延生成して両端を二分探索します。

| 条件（20 万行） | 変化 |
|------|------|
| `id > 199990` | 21.4ms → **2.4ms** |
| `id BETWEEN 5 AND 20` | 26.5ms → **1.6ms** |
| `s > 'v99998'` | 28.6ms → **1.0ms** |

型が混ざった列（比較側が数値へ寄せるので生値の並びと食い違う）と照合順序付きの列は
使いません。また**区間が広いと索引の方が高い**ので、相異なりキーの 1/4 を超える
範囲は全表走査に任せます（`id > 100000` は索引 43.7ms / 走査 13.6ms でした）。

**`MIN`/`MAX` が全件を並べ替えていた**

端の 1 個を取るために配列を `sort` していたので、1 度のなめに変えました
（`MAX(s)` 20.9ms → 8.9ms、`GROUP BY` 併用 62.6ms → 45.9ms）。
比較は生値のままなので、文字列の `MIN`/`MAX` は文字列として並びます。

**文字列プールが `UPDATE` のたびに伸び続けていた**

`TEXT` の値を書き換えると古い文字列が参照されないままプールに残ります。
5,000 行の列を 6 回更新するとプールが 35,000 件（生きている値の 7 倍）になり、
**増える一方だったので最後は 1,677 万件の上限で挿入が失敗**していました。
`VACUUM` が 1.4ms で完全に戻せるのに誰も呼んでいませんでした。
生きている値の 4 倍を超えたら自動で回収します。回収は**文が成功した後・
トランザクションの外**だけ: 文単位ロールバックと `ROLLBACK` はプールの長さを控えて
巻き戻すので、その控えが生きている間に詰めると復元が壊れます。

**`IN (副問い合わせ)` が毎行リストを作り直していた**

副問い合わせの行はリテラルとして SQL 文へ展開されるので、`IN (SELECT g FROM t)` は
20 万個のリテラルを式コンパイルへ通し、**1 行評価ごとに 20 万要素の配列を確保**して
いました（810ms。同義の `JOIN` は 3ms）。`IN` は集合の所属を見るだけなので重複は
答えに影響しません（`NULL` も 1 つ残れば三値論理は保たれます）。相異なり 1,000 個へ
落として **810ms → 45ms**。長いリストは初回に集合へ起こして走査もやめました。

**`EXPLAIN` の見積りを索引から出すようにした**

固定の選択率（等価 0.1 / 範囲 0.33）だったので `id > 49990` を 0 行、
`id > 25000` を 16,500 行（実際 25,000）、`id > 49000 AND s='v7'` を 330 行
（実際 20 — 16.5 倍のずれ）と見積っていました。索引は相異なりキー数を知っており、
並べたキーの位置から範囲の広さも判るので、**`ANALYZE` のような統計収集を挟まずに
常に最新の実測値**で出せます。試した 8 例すべてで実際の件数と一致しました。
併せて 2 つの既存の欠陥を直しています: 範囲の索引スキャンを「1 キー分」と見積って
いた（後段の `FILTER` と掛かって 0 行になっていた）／索引が既に絞った条件の選択率を
`FILTER` 段で二重に掛けていた。

**まだ手を付けていないもの**

- 索引の無い列を相関副問い合わせで引くと、相異なり値ごとに全表走査（400 通り × 20 万行で 4.8 秒）。索引があれば 21ms で済むので、一般解は半結合への非相関化。
- 式の中の十進演算（`price * 3`）は二進浮動小数のまま。
- 全文検索（`MATCH ... AGAINST`）に転置索引が無く毎回走査。
- 自動保存が変更のあった表を丸ごと直列化（20 万行で 86ms / 12.3MB）。粒度が表単位。
- `exportSQL` が全体を 1 本の文字列にする（20 万行で 5.5MB）。
- 実行中クエリの中断（単一スレッドなので実行をワーカーへ移す設計変更が必要）。
- Tailwind の自前同梱、バンドラからの直接 `import`。

---

### v1.36 — 実行計画・数値の正しさ・配信サイズの見直し

コードを git 管理下に置いたのを機に、実測で裏を取った上で「遅い」「黙って誤る」箇所を
まとめて直した回です。テストは `test-suite-v65`〜`v68`（407 件）を追加し、
全体で 53,064 件になりました。

**結合と索引 — 書き方ひとつで数百倍違っていたところ**

- **複合キーの等価結合がハッシュ結合に乗るようになった。** ハッシュ結合の判定が
  「ON がちょうど 1 個の `a.x = b.y`」という正規表現だったため、`AND` が 1 つ付くと
  入れ子ループへ落ちていた。正規化されたスキーマで最も普通に書かれる複合主キーの結合が
  30,000 × 3,000 行で **16,330ms**（`USING (k1, k2)` は 17,742ms）かかっていた。
  深さ 0 の `AND` で分解し、等価な連言を鍵に、残り（不等号・関数・単表の絞り込み）を
  検算条件へ回す。**81ms / 59ms** になった。20 万行の自己結合 + `x.id < y.id` は
  22,370ms → **163ms**。
  ハッシュは候補を絞る網としてだけ使い、拾った候補は入れ子ループと同じ ON 式で
  検算するので、答えは変わらない。
- **WHERE に連言があっても索引を使うようになった。** 索引経路の判定も WHERE 全体に
  先頭末尾を固定した正規表現だったため、`WHERE id = 4242 AND g = 42` が 20 万行を
  舐めていた（23.9ms → **2.3ms**）。索引で引いた等価も含めて WHERE を元の形のまま
  再評価するので、索引の有無で答えが動かない。
  どの索引を使うかは主キー / UNIQUE を優先し、次に相異なりキー数の多い方を採る。
- **`ORDER BY ... LIMIT n` が全件を並べ替えなくなった。** 上位 k 件だけを持ち回る
  選択に変えた（20 万行から数件で 112.9ms → **44.4ms**）。並びは「安定ソートして
  先頭 k 件」と一致する。`TOP n PERCENT` / `WITH TIES` / `DISTINCT ON` は
  全体件数や打ち切り位置の先が要るので従来どおり全件を並べ替える。

**数値と時刻 — 間違った答えを黙って返していたところ**

- **`SUM` / `AVG` が十進で合うようになった。** `DECIMAL(10,2)` の列に 0.10 と 0.20 を
  入れて `SUM` を取ると 0.30000000000000004 になり、`SUM(price) = 0.30` が偽だった。
  値から小数桁を読み取って整数で足し、最後に割る。1,000 行の 0.01 がちょうど 10.00 に
  なる。桁が深すぎる / 大きすぎる場合は倍精度の素朴な和へ戻す。
  なお式の中の演算（`price * 3`、`a + b`）は今も二進浮動小数のままで、そこまで
  正確にするには格納形式を桁ずらし整数へ変える必要がある。
- **2^53 を超える整数を黙って丸めなくなった。** `9007199254740993` が `...992`、
  `9223372036854775807` が `...776000` として格納されていた。外部システムの ID
  （Snowflake ID・Java の long・伝票番号）を入れた瞬間に壊れる類なので、
  INTEGER 系の列は範囲外の値を断り、「桁を保つなら TEXT へ」と案内する。
  `CAST` は丸めた数の代わりに NULL を返す。
- **`AT TIME ZONE` / `CONVERT_TZ` が IANA 名を取るようになった。** `'Asia/Tokyo'` が
  `Unknown time zone` で落ちていた。`Intl` から引き、オフセットは**その瞬間**に対して
  求めるので夏時間も正しい（`America/New_York` は 1 月 -5 / 7 月 -4、
  `Europe/London` は 8 月 +1）。従来の `'UTC'` / `'+09:00'` / 略称は変えていない。

**配信サイズ・運用**

- **テストを既定で読み込まなくなった。** `LuminaDB.html` が 72 本のテストを
  `<script>` で静的に読んでいたため、画面を開く全員がペイロードの 62% を
  テストのために払っていた（`js/tests` は 2.49MB で、製品コード 1.59MB より大きい）。
  並びを `js/tests/manifest.js` に移し、`runtest` / `?autotest=1` のときだけ
  動的に読む。開いたときに読むのは **99 ファイル 4.02MB → 27 ファイル 1.53MB**。
- **画面から実行する文に既定の時間上限（30 秒）を置いた。** JavaScript は単一スレッドなので
  同期実行中のクエリを Cancel ボタンで止めることはできず（クリックが処理されない）、
  上限が無いと暴走したクエリでタブが固まったままになる。
  `SET statement_timeout = 0` で外せる。エンジン既定は 0（無制限）のままで、
  ライブラリとして使う場合の方針は呼ぶ側に委ねる。
- **`package.json` / 型定義 / `index.mjs` を追加した。** Node / Bun から
  `import { createDatabase } from 'luminadb'` で使える。ブラウザ側は従来どおり
  `<script>` 読み込み（ビルド不要という前提を変えないため）。

**その他の修正**

- **列定義の後ろの表オプションが本当に無視されていた。** `CREATE TABLE` の列定義を
  貪欲一致で切っていたため、`CREATE TABLE t (id INT) PARTITION BY RANGE (id)` では
  列定義が `id INT) PARTITION BY RANGE (id` になり、後置オプションが空になっていた。
  そのため「受理はするが効いていない」ことを伝える警告（`NO_PARTITIONING` /
  `NO_SYSTEM_VERSIONING` / `TABLE_OPTIONS_IGNORED`）は一度も出たことが無かった。
  対応する括弧を数えて切り直し、警告が出るようにした。
- **索引経路の型寄せが `INT` の列を素通りしていた。** 索引のキーは生値なのに比較側
  （`__eq`）は型を寄せるため、宣言型の名前で補正していたが `'INTEGER'` と `'FLOAT'`
  しか見ていなかった。`INT` と書いた列では `WHERE g = '2'` が索引経路で 0 件、
  全表走査で 2 件という食い違いが出ていた（`BIGINT` / `DECIMAL` / `REAL` も同様）。
  型名を数え上げるのをやめ、`__eq` が同値と見る候補値を全部引いて和集合を取る。
- 生の NUL バイトで書かれていた複合キーの区切りを `\0` エスケープ表記にした。
  git が text/binary を先頭 8000 バイトの NUL で判定するため、`test-suite-v18.js` が
  binary blob として扱われ、行差分も行履歴も取れなくなっていた。
- 改行コードを LF に統一し、`.gitattributes` で固定した。

---

### v1.35 — 画面表示の日本語 / 英語切り替え（既定は日本語）

サイドバー見出しの **「JP / EN」** で表示言語を切り替えられるようにした回です。
選択は `localStorage` に残り、次に開いたときも同じ言語で表示されます。
テストは `test-suite-v64.js`（34 件）を追加し、全体で 52,655 件になりました。

**作り**

- **日本語を原文として扱う**。HTML には日本語がそのまま書いてあり、英語のときだけ辞書
  （[`js/ui/i18n-en.js`](js/ui/i18n-en.js)、330 件）で差し替える。JS を切っても日本語は読める。
- 辞書のキーは**日本語の原文そのもの**。キー名を別に発明すると HTML と辞書の二重管理になり、
  必ずどちらかがずれる。
- 静的な文言は DOM を歩いて拾う。**子が `<span>` / `<code>` / `<kbd>` だけの要素は
  innerHTML ごと 1 単位**として訳すので、「文の途中に `<code>` が挟まる」文章が切れない。
- JS が組み立てる文言は `i18nT('日本語')` で包む。`i18nT('保存エラー: {0}', e.message)` の
  ように `{0}` へ引数が入り、**訳文では語順を変えてよい**
  （例: 「{0} 表中 {1} 表を書き込み」→「{1} of {0} tables written」）。
- 原文は WeakMap へ控えてから差し替えるので、何度でも往復できる。

**訳さないもの**

| 対象 | 理由 |
| --- | --- |
| 表名・列名・結果セル・クエリ履歴・実行ログ・ER 図 | 利用者のデータであって画面の文言ではない。`data-i18n-skip` を付けた領域は走査しない |
| SQL エンジンのエラーメッセージ | 元から英語。DB のエラーは言語設定で変わらないほうが検索しやすい |
| トースト（出した後の通知） | 出す時点の言語で組み立てる。後から訳し直しても意味が無い |

**踏んだ落とし穴**

- 翻訳関数を短く `t()` にしたら、このコードベースが `t` を「テーブル」「タブ」の局所変数として
  広く使っているため**静かに隠れて `TypeError`** になった。`tr()` も `<tr>` 用の変数と衝突したので
  `i18nT()` に落ち着いた。
- 一度英語にした要素は「日本語らしさ」では見つからないので、**原文を控えてある要素は
  いまの中身が何語でも走査対象に含める**必要がある（でないと日本語へ戻せない）。
- 辞書は読みやすさのため改行入りで書いてあるが、引くときは空白を畳んだ形になる。
  畳んだ別名を作っておかないと**複数行の SQL 例だけが黙って訳されない**。

**追加したテスト**（`test-suite-v64.js`）— 既定が日本語であること、切り替えが `localStorage` と
`<html lang>` に反映されること、**英語表示で日本語が残っていないこと**（通常画面・全モーダル・
データ画面の 5 タブ・コマンドリファレンスを機械的に走査）、日本語→英語→日本語で原文へ戻ること、
5 往復しても崩れないこと、`i18nT` の引数差し込みと未登録語のフォールバック、
辞書の `{0}` の個数が原文と訳で一致すること、表名・結果セル・エンジンのエラーが訳されないこと。
