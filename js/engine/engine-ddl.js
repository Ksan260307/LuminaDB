    ...
    const LUMINA_FN_REGISTRY = {
        'String': 'UPPER LOWER LENGTH CHAR_LENGTH CHARACTER_LENGTH OCTET_LENGTH BIT_LENGTH CONCAT CONCAT_WS SUBSTRING SUBSTR MID SUBSTRING_INDEX LEFT RIGHT LPAD RPAD TRIM LTRIM RTRIM REPLACE REVERSE REPEAT INSTR LOCATE POSITION ASCII CHAR SPACE STRCMP ELT FIELD INITCAP UCASE LCASE FORMAT HEX UNHEX BIN OCT CONV QUOTE SPLIT_PART TRANSLATE INSERT SOUNDEX',
        'Regexp': 'REGEXP_REPLACE REGEXP_SUBSTR REGEXP_LIKE REGEXP_COUNT',
        'Numeric': 'ABS CEIL CEILING FLOOR ROUND TRUNCATE TRUNC MOD SIGN POWER POW SQRT CBRT EXP LN LOG LOG10 LOG2 PI RAND RANDOM SIN COS TAN COT SINH COSH TANH ASIN ACOS ATAN ATAN2 DEGREES RADIANS GREATEST LEAST BIT_COUNT CRC32 FORMAT_BYTES',
        'Date & Time': 'NOW CURRENT_TIMESTAMP SYSDATE UTC_TIMESTAMP CURDATE CURRENT_DATE UTC_DATE CURTIME CURRENT_TIME DATE TIME YEAR MONTH DAY DAYOFMONTH HOUR MINUTE SECOND DAYOFWEEK DAYOFYEAR WEEKDAY WEEK WEEKOFYEAR QUARTER MONTHNAME DAYNAME LAST_DAY DATEDIFF DATE_ADD DATE_SUB ADDDATE SUBDATE EXTRACT TIMESTAMPDIFF TIMESTAMPADD DATE_FORMAT STR_TO_DATE UNIX_TIMESTAMP FROM_UNIXTIME SEC_TO_TIME TIME_TO_SEC MAKEDATE MAKETIME TO_DAYS FROM_DAYS DATE_TRUNC',
        ...
        'Null & Flow': 'COALESCE IFNULL NVL NULLIF IF IIF CASE CAST CONVERT',
        'Encoding & Hash': 'MD5 SHA1 TO_BASE64 FROM_BASE64 INET_ATON INET_NTOA',
        'Aggregate': 'COUNT SUM AVG MAX MIN GROUP_CONCAT STRING_AGG ARRAY_AGG STDDEV STDDEV_POP STDDEV_SAMP VARIANCE VAR_POP VAR_SAMP MEDIAN BIT_AND BIT_OR BIT_XOR BOOL_AND BOOL_OR CORR COVAR_POP COVAR_SAMP ANY_VALUE JSON_ARRAYAGG JSON_OBJECTAGG MIN_BY MAX_BY COUNT_IF PERCENTILE_CONT PERCENTILE_DISC GROUPING',
