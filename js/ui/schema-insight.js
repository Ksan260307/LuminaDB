    ...
    function renderProfile(tbl) {
        ...
        document.getElementById('profileNote').textContent = t.rowCount === 0 ? '空のテーブルです' : '';
        ...
        stats.forEach(st => {
            ...
            nameBtn.title = 'この列の分布を出すクエリをエディタへ';
    ...
    function openProfile(tbl) {
        if (!db.tables[tbl]) { showToast(`テーブル '${tbl}' がありません。`, true); return; }
        renderProfile(tbl);
    ...
    function renderEr() {
        ...
        document.getElementById('erNote').textContent = edges.length === 0 ? '外部キーはまだありません' : '';
        if (names.length === 0) {
            body.innerHTML = '<div class="p-8 text-center text-gray-400 text-sm">テーブルがありません。</div>';
            return;
        }
    ...
    document.getElementById('erCopyBtn').addEventListener('click', () => {
        ...
        if (!svg) { showToast('図がありません。', true); return; }
        const text = new XMLSerializer().serializeToString(svg);
        if (navigator.clipboard && navigator.clipboard.writeText) {
            navigator.clipboard.writeText(text)
                .then(() => showToast('SVG をコピーしました。'))
